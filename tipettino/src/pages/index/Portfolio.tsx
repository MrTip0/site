...
export default class Portfolio extends React.Component {
    ...
    Title() {
        return (
            <h1 className="text-4xl text-center uppercase text-primary-500 mt-6">Portofolio</h1>
        )
    }
...
class MoreInfo extends React.Component {
    ...
    render() {
        return(
            <div className="flex flex-col items-center group">
                <img onClick={()=> this.toggle()} src={process.env.PUBLIC_URL + "/assets/img/portfolio" + this.props.proj.imgUrl} alt={this.props.proj.title + " image"} className="w-96 rounded-lg shadow-md m-3 border-0 border-primary-300 hover:border-4 transition-transform duration-150 transform group-hover:scale-105"/>
                <div hidden={this.state.hidden} className={"bg-primary-200 w-96 rounded-lg transition-transform duration-150 transform group-hover:scale-105"}>
                    <div className="m-5 flex flex-col items-center">
                        <h1 className="text-xl text-primary-600 uppercase mb-3">{this.props.proj.title}</h1>
                        <this.GitHub url={this.props.proj.github}/>
                        <this.Download downloads={this.props.proj.download}/>
                        <button className="mt-4 text-gray-500 text-sm transition-transform duration-150 transform hover:scale-105" onClick={()=> this.toggle()}>hide</button>
                    </div>
                </div>
            </div>
        )
    ...
    GitHub(props: any) {
        if (props.url !== 'none') {
            return (
                <button className="bg-secondary-200 rounded border-2 border-secondary-300 transition-transform duration-150 transform hover:scale-105" onClick={()=>{window.location.href = props.url}}>
                    <div className="m-2 w-44 flex flex-row content-center">
                        <div className="flex flex-col content-center mr-2 ml-2">
                            <img src={GhLogo} alt="github logo" className="w-5 h-5" />
                        </div>
                ...
                <Menu>
                    ...
                    <Menu.Items className="m-4 bg-secondary-200 rounded flex flex-col items-center border-2 border-secondary-300">
                        {downloads.map((value) => {
                            return (
                                <Menu.Item>
                                    <button className="text-secondary-500 m-2 w-44 flex flex-row items-center" onClick={()=>MoreInfo.download(value.link)}>
                                        <img src={Logo(value.platform)} className="w-5 h-5 ml-2 mr-3" alt={value.platform + " logo"}/>
                                        <p>{value.platform}</p>
                                        </button>
                                </Menu.Item>
                            );
