    ...
    handleSubmit(event: any) {
        if (!this.state.sent) {
            ...
            xhttp.onload = () => {
                ...
                if (result === 'success') {
                    this.showMessage('Message sent succesfully', 'you can close this window')
                } else {
                    this.showMessage('An error has occured', 'the server has response: ' + result)
                    this.setState({
                        sent: false,
                        disabled: false
                    })
    ...
    render() {
        return(
            <div className="h-auto bg-secondary-200 w-min-full flex flex-col">
                <h1 className="text-4xl text-center uppercase text-primary-500 mt-5">Contact me</h1>
                ...
                <Alert open={this.state.openAlert} close={()=>this.setState({openAlert: false})} title={this.state.alertTitle} description={this.state.alertDescription}/>
            </div>
        )
    }
}
                ...
                <Dialog.Overlay className="fixed inset-0 bg-secondary-50 bg-opacity-70 flex flex-row items-center">
                    <div className="flex flex-col items-center w-screen">
                        <div className="flex flex-col w-80 rounded bg-primary-200">
                            <Dialog.Title className="w-auto m-5 text-xl font-semibold text-center">{props.title}</Dialog.Title>
                            <Dialog.Description className="w-auto m-5 text-base font-semibold text-center">{props.description}</Dialog.Description>
                            <button ref={button} className="h-8 w-auto rounded border-2 border-primary-500 m-5" onClick={props.close}>Close</button>
                        </div>
                    </div>
                </Dialog.Overlay>
        </Dialog>
